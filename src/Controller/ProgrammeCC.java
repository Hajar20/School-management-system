...
@WebServlet("/ProgrammeCC")
public class ProgrammeCC extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<ConCont> lCC=ccDAO.selectCC(idUser,idClasse);
		request.setAttribute("lcc" , lCC);
		request.setAttribute("lMod" , listModule);
		request.setAttribute("idUser" , idUser);
		request.setAttribute("idClasse" , idClasse);
		RequestDispatcher dispatcher = request.getRequestDispatcher("ProgrammeCC.jsp");
